...
{\it
	...
	Ostatnią grupę opisywaną w tym rozdziale stanowią standardy dotyczące zapewniania bezpieczeństwa komunikacji. W systemach typu SOA może istnieć wymaganie dotyczące zagwarantowania poufności danych wymienianych między komponentami systemu, tak aby krytyczne dane jak np. hasła nie mogły zostać odczytane przez nikogo poza ich odbiorcą. Innym wymaganiem jest zapewnienie integralności danych, czyli uniemożliwienie ich podmiany na skutek ataku lub awarii. Mechanizmy zapewniające te cechy komunikacji mogą działać na poziomie kanału transmisji pomiędzy nadawcą i odbiorcą lub na poziomie wiadomości. 
...
\autsection{Przykłady standardów zarządzających uwierzytelnianiem i autoryzacją użytkowników}{Krzysztof Wilaszek}
